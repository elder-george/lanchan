...
It was originally developed (during a Hackathon at Tableau) for use with HipChat which doesn't allow posting files but can provide previews for URLs.

Now it's mostly a memento for me.
...
- the files are stored in the `uploaded-files` folder "as is"; no metadata 
